...
**Drain sizing, decision sheet, footprint authority, falsifiability (Kit-Version 2026-08-30.1, from the third DragonWar report of 2026-08-29):** the report's six bugs all reproduced against its artifacts (`Story 1.9` collision at `epics.md:662`; `owner:burndown=30` against cap 8; `readonly never[]` behind a recorded ADR `pass`; a phantom HEAD in a clarification; five one-time footprint widenings; ~1 human round trip per story). Two findings the report missed changed the shape of the fixes. (1) BUG 1 is a misread the kit invited, not a missing authority: Rule 5 already says the lead amends `epics.md`, Runner-Mode Deltas already say the runner is the lead, and the kit itself mandates runner edits to `epics.md` (X.0 insertion, the burn-down charter) — so Rule 11 is rewritten as "concurrency is not authority" (contended paths from the DAG; own story blocks + artifacts always in authority; `amendments:` / `footprint_extensions:` in the contract, verified post-hoc as `amendments_verified`), and Rule 5 gains two tiers (apply-and-report vs ask-first). No `planning_authority` yaml key. (2) The burn-down pile decomposed into five classes, only one of which is burn-down work: 17 LOWs routed instead of closed (a Rule 15 table hole — "in-story footprint" where it meant the epic's), 3 MEDs the reviewer routed instead of fixing, 4 `escalated` decisions, 7 legacy/human-owned migration entries, ~8 genuine. Hence: Rule 15 "in-footprint = the epic's", "`burndown` never owns a LOW", a human-owned row (`decision-pending note=human=`), migration triage by class; Rule 17 gains `burndown_story_max` (default 12) + explicit overflow + drain ratio, sets `escalated`/`decision-pending` aside, and adds (5) the **epic-close decision sheet** at SC-4 (`decision_sheet_recorded`; `by=merge_gate` trailer lines; human-owned items → `sprint-status.yaml` `action_items` via BMAD's `sprint_status.py update --add-action`, entry closed `dropped note=action_item`); spec validation gains a decision-dependency check (a spec built on a `decision-pending` entry is an intent gap now, not at the merge gate — DW-2's tick-rate ratification sat undecided for seven stories). Rule 19 (falsifiability: one demonstrated mutation per AC, applied/observed/reverted/recorded; the lead's ADR gate may not record `pass` without one) is carried into the QA block, the CR block, the `verification-gap` layer override, and telemetry (`mutations_demonstrated=`, `mutations=`, `result=unverified`). The upstream `blind-hunter` quota ("find at least ten … never an empty list") is softened in both overrides — measured 6–28 dismissed per review pass — while keeping "look for what's missing". `Story N.9` → `N.<next free number>` at every site; every quoted sha is `rev-parse`d at write time and `cat-file -e`'d by the reader (`report_error` / `runner_report_error`); Permission Mode and README describe the loop as supervised (~1 round trip per story, zero false halts). Verified: both tomls merge through `resolve_customization.py` (4 layers by id, Rule 18 ×4, Rule 19 ×1, quota gone, handoff Rule 18 ×1); `sprint_status.py update --add-action` appends `status: open` + stable id on a scratch copy; all parallel-kit anchors still present once; fences balanced.

**The spine is the registry; audit against the installed tree and BMAD `main` (Kit-Version 2026-08-30.2):** two audits ran after 30.1 — one walked every contract the kits assume against the installed 6.11.0 skills (a fresh `render_skill.py` snapshot of `bmad-build-auto` included), the other read docs.bmad-method.org, the release list, and `bmad-loop`. Findings and what changed: (1) **BMAD has no ADR concept** — no skill or doc page mentions `docs/adr/`; its decision registry is the `bmad-architecture` spine (`ARCHITECTURE-SPINE.md`, `### AD-n — {decision}` blocks with Binds/Prevents/Rule, `lint_spine.py`, an Update intent that returns `conflicts_with_prior_decisions`, `.memlog.md` as the authority). Rule 6 is rewritten around the spine ("AD violations are HIGH"; resolution order newest final spine → newest → `*architecture*.md` → none, logged `spine_resolved`), new **Rule 20** writes every decision with architectural weight (a seam, a convention, a pinned dependency/licence, a numeric invariant) into the spine at the moment it is made — light path by default (new/amended AD keeping ids stable, Conventions/Stack row, `memlog.py append`, `lint_spine.py`, commit, pre-warm), `bmad-architecture` Update intent (interactive) when a decision conflicts with an existing AD; in a parallel run every runner writes into its own worktree's spine at the moment of the decision too (a decision held for the completion contract would be lost to a crash, an upgrade, or a declined merge — the user's objection during review); the only shared state is the next `AD-n`, claimed from an orchestrator-seeded `spine-next-id` counter behind a `spine.lock` (Rule 12 mechanics), and SC-4-P reconciles the two spines deterministically (both sides' AD blocks in id order, `lint_spine.py` after; the `.memlog.md` is union-merged). Cycle-log identifiers (`adr_*`) keep their spelling. (2) **Drifts found by the skills audit and fixed:** build-auto's dirty-tree halt has no canonical string (new free-text row in the blocking-condition table); the CR block contradicted itself on the spec's frontmatter `status:` (1c rewritten); code-review's sprint-status sync is an LLM in-place YAML rewrite (pre-answered to `sprint_plan.py generate --set`, plus `SPRINT_PLAN validate` after `cr_complete`); the spec template deletes `## Design Notes` / `## Verification` for trivial stories (spec gate re-creates them, `sections_created=`); a review-stage `intent gap` saves the attempted diff as a patch (re-dispatch now `git apply`s it and resumes at `in-review` instead of re-implementing); two un-pre-answered CR prompts (subagent permission, deferral reason); `baseline_commit` accepted alongside `baseline_revision`; the renderer-HALT claim narrowed to the two rendered skills; the `spec-<key>-2.md` rule corrected (a `draft` spec resumes in place); stray `## Deferred from:` bullets harvested at `cr_complete` (`ledger_stray_bullets`). (3) **Time bomb on BMAD `main` (unreleased; v6.11.0 is still latest):** code-review's layer placeholder `{diff_output}` becomes `{diff_file}` and triage becomes a five-way verdict; because an id-keyed override replaces the whole layer, pasted 6.11.0 text would pin a dead placeholder. File 2 / File 5 are now **derived from the installed `customize.toml`** (copy verbatim, apply the kit's three transformations); Step 4 check 1d diffs every `{placeholder}` in an override against the installed skill; Rule 15's CR bullet says the table governs whatever the skill's buckets are called. (4) Documented and unused, deliberately left for a later version: `activation_steps_prepend` for Rule 13, `bmad-correct-course` for tier-2 amendments, `bmad-project-context record`, `bmad-review` lenses, folder+id dispatch from `SPEC.md` + `stories.yaml` (the docs' direction of travel; `epics.md` + `sprint-status.yaml` remains first-class and undeprecated). README: the kit is the unattended variant of the Phase-4 chain (the docs' "AI coding session as orchestrator"), and `bmad-loop`'s ledger must never share a `deferred-work.md` with this kit's (same `### DW-n:` heading, different grammar). **Fourth DragonWar report (2026-08-30, filed against 30.1; folded into 30.2):** zero runner clarifications across two stories under the Rule 11 split, but three beliefs the pipeline never checked — (a) routed ledger entries reach a story only at `ledger_adjudicated`, after its design is fixed (fifteen on one unplanned story): Rule 17 (1b) "routed means planned" — a `DW-n:` acceptance bullet in the receiving story's `epics.md` block (in-epic by the lead/runner immediately, cross-epic by the sequential lead or the orchestrator post-merge, bounded by `routed_story_max`, `ledger_routed_planned`) plus the story's `LEDGER slice <key>` as a plan-spawn inbox that spec validation checks (`owned_ledger= addressed= declined=`); (b) nothing validated a ledger owner key (two entries orphaned on a retitled story's old slug): `ledger.sh` now refuses unknown owners on `new`/`append`, `load` reports `owner_unknown=` and marks them, `slice unknown` lists them, and `ledger_load` repairs by unique `<epic>-<story>-` prefix or halts (`ledger_owner_repaired`); a retitling Rule 5 amendment re-owns in the same commit; (c) no gate read CI (a branch red for ~16 h passed two story pipelines): `runtime_gate ci=gh|none|unavailable`, `committed … ci=pending run=` recorded write-ahead and resolved before the next plan spawn / burn-down / merge / resume (`ci_resolved`), red = HIGH via the failed-smoke path, SC-4 and SC-4-P require `ci_green_verified` on the head or log `merge_gate_blocked reason=ci_red`, the runner contract carries `ci:`; plus the smoke gate's platform-asymmetry note and `remove-epic-worktree.sh` treating deregistered+empty as success on Windows.

**The rework loop has an exit (Kit-Version 2026-09-11.1, from the OcuPilot field report of 2026-09-11):** the report's central claim — six reasonable rules combining into a loop with no reachable exit — reproduced by construction against the installed BMAD 6.12.0 skills and the kit text (13 stage spawns, 9 implement passes, 3 reviews, zero HIGH since the third review, a cap override with no close condition). Changes: Rule 15 splits the MED fix-now row — an in-story MED is patched by the reviewer in-pass or takes the story's ONE bounded iteration on its first review only; an in-epic MED is `routed` to the owning story's key with its bullet; **a re-review that finds no HIGH closes the story**; decisions on a reviewed story become bullets on later stories, never a re-open; a cap override is a contract (`items=` + `close=next_review_without_high`). Re-reviews are scoped to `<rework baseline>..HEAD` (the skill already uses `baseline_commit` as its diff baseline; the kit had reset it to the original baseline, so round 3 re-read 7,247 lines for 0 HIGH). Rule 19: a stale or missing `mutation:` line on a test that exists is a LOW the reviewer closes in-pass; the implement stage and every rework write the line when they change a pinning test (the loop had been manufacturing its own next round — 11 of 62 self-review mediums). Rule 18 (3) propagates test-run serialization to every depth and the spec gate flags `mutates_shared_runtime` stories onto a throwaway environment (18 concurrent test-runner calls left a database mounted with no file). Counting: `cr_complete` severities are root-cause entries with `rows=` and `unresolved_high_med=` alongside, and the model-tier checkpoint escalates on `unresolved_high_med` (gross high+med scored 8, 7 and 10 on three one-pass stories against a threshold of 2). build-auto's follow-up convergence clause is pre-answered on rework re-dispatches (upstream sets `followup_pass` only for a `done` spec; the lead's `in-progress` re-open always ran as a first pass, and the kit logged `followup_review_recommended` without reading it). Not adopted: the report's D8 (evidence labelling on ledger writes) — tentative in the report, one story's evidence; watch for recurrence. Report corrections: wall clock about 40 h, not 64; the appendix regexes give 29 duplicates and 11 Rule 19 rows, not 31 and 16.

**Burn-down story numbering (Kit-Version 2026-09-13.1, user report 2026-09-13):** leads kept chartering the burn-down story as `N.9` because 2026-08-30.1 left "conventionally `N.9`" as the headline with the real rule as a fallback, and its fallback, "the next unused number", can fill a gap below an existing story. The story is now `N.<M+1>`, M the highest existing story number under Epic N across `epics.md` and `sprint-status.yaml` (gaps irrelevant, `N.0` counts), reused if an "Epic N burn-down" story already exists; no conventional number remains anywhere in the kits (base, parallel, README), and Step 4 checks that only the anti-pattern still mentions `N.9`. The parallel kit's one mention is updated and its version bumped in step.
