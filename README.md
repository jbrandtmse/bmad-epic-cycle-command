...
- **`.claude/commands/epic-cycle.md`** — the command itself. Given an epic range (e.g. `/epic-cycle 1-3`), a lead session drives the full BMAD story pipeline per epic: headless sprint planning (readiness gate) → `bmad-build-auto` **plan** spawn (Opus, `Halt after planning.`) → lead validates the spec → `bmad-build-auto` **implement** spawn (Sonnet; implements, self-reviews, commits) → QA test generation → adversarial `bmad-code-review` (Opus) → bounded rework loop (each re-review scoped to the rework; a re-review with no HIGH closes the story; max 3 iterations) → per-story smoke → commit/push, plus per-epic branch management, retrospective, and merge gates. Every stage is spawned with an explicit stage→model map; the lead owns `sprint-status.yaml` writes through BMAD's deterministic `sprint_plan.py`.
- **`_bmad/custom/skill-rules.md`** — a cross-cutting rules registry (integration ACs, real-runtime test evidence, architecture-decision enforcement, unattended-menu protocol, finding-disposition bar, clean-tree-before-dispatch, the ledger drain, containment, falsifiability, decisions-into-the-spine, and more) loaded by every BMAD skill via `persistent_facts`. Rule 19 (falsifiability) requires one demonstrated mutation per acceptance criterion — name it, apply it, observe red, revert — from QA, from the code-review `verification-gap` layer, and from the lead's own architecture-decision gate; a pass whose red was never observed is `unverified`, not `pass`. A pinning test that exists but lacks a `mutation:` line is a LOW the reviewer closes in-pass; only a missing pinning test or an unfalsifiable assertion is a MED (a 2026-09-11 field report found the rework loop manufacturing its own next round from lines its own test changes had made stale). Rule 15 gives the loop an exit: a HIGH re-opens a story at any review, an unpatched in-story MED only at its first review, in-epic MEDs route to the owning story, and a re-review that finds no HIGH closes the story with the rest routed to named owners. The decision registry is BMAD's own **architecture spine** (`ARCHITECTURE-SPINE.md`, the `AD-n` blocks `bmad-architecture` writes) — there is no separate ADR folder: Rule 6 makes an AD violation HIGH, and Rule 20 writes every decision the pipeline makes with architectural weight (a clarification answer, a planning amendment, a decision-sheet call) back into the spine at the moment it is made, linted with BMAD's `lint_spine.py`, so it reaches the next epic's plan stage through the compiled epic context.
- **`_bmad/scripts/ledger.sh`** — the deferred-work ledger tool (bash + awk). Every finding the pipeline defers is filed with an owner, closed at the owning story's `ledger_adjudicated` gate, swept at the epic's burn-down gate (which can charter a burn-down story numbered one past the epic's highest existing story, never a fixed `N.9`), and re-decided — never relabelled — at the next epic's Story X.0. The lead reads the ledger only through `load`/`slice`/`show` and writes only through `new`/`append`, so the file stays small, union-merge-safe, and machine-countable (a field report of a 1,588-entry, never-shrinking ledger motivated this). The drain is bounded in both directions: a LOW is fixed at emission or closed with a probe — `burndown` never owns one — the chartered burn-down story takes at most `burndown_story_max` entries by priority with the overflow re-owned explicitly, and every epic logs its drain ratio. Anything only a human can decide (`escalated` MEDs, product calls, playtests, ratifications) never goes to a burn-down story: it is presented once, with a recommended disposition, on the **epic-close decision sheet** at the merge gate — a gate you already attend — and a spec that would build on an undecided item halts at spec validation instead. Routing is planned, not merely recorded: an entry routed to a story becomes a `DW-n:` acceptance bullet in that story's `epics.md` block and arrives at its plan spawn as an inbox to address or decline, so adjudication checks a promise rather than discovering a surprise; `ledger.sh` refuses an owner key the tracker does not know (a retitled story would otherwise orphan its entries silently). And CI is a gate: every push's run is recorded on the `committed` entry and resolved before the next story starts, and a red branch head blocks the merge question — the local suite is evidence about the lead's machine, not the platform the project ships from.
...
- **Runtime-lock protocol** — claim-file locks serializing stages that touch a shared live runtime (smoke, QA, architecture-decision verifications).
...
- **Footprint = concurrency, not authority (Rule 11)** — the epic footprint says what might collide with a concurrently running epic; the runner is the lead for its epic and owns its story blocks in `epics.md`, its artifacts, and any non-contended path (reported under `footprint_extensions:`). Intent-preserving planning amendments are applied and listed under `amendments:` for post-hoc orchestrator verification; product-changing ones, contended paths, and other epics' stories still halt. (From a 2026-08-29 field report: five one-time "widenings" in three stories, four of nine clarifications authorization-only.)
- **Containment (Rule 18)** — a returned agent is finished (`SendMessage` to any returned agent is forbidden at every depth; `bmad-build-auto`'s own "re-engage the subagent" text is pre-answered as "cannot"); every agent returns exactly once; the base kit writes the git/CI/`SendMessage` prohibitions into `bmad-build-auto`'s handoff and review-layer prompts via `_bmad/custom/*.toml`, so depth-3 agents carry them by construction; an orphan-writer protocol for when it happens anyway. Rule 18's propagated list also serializes stateful test runs against a shared runtime — one class or suite per call — after a 2026-09-11 field report in which 18 concurrent test-runner calls left an IRIS instance with a database mounted over a deleted directory. The model-tier checkpoint now evaluates on every project (the policy file is needed only to apply a change) and escalates on `unresolved_high_med` (root-cause entries the reviewer could not resolve in its own pass), not on the gross count a four-layer review produces on healthy stories.
...
**Supervised, not unattended.** The permission mode is unattended; the loop is not. On a greenfield epic expect roughly one human round trip per story (a 2026-08-29 field report measured 9 clarifications and 7 decisions across 8 stories, with zero false halts) plus one decision sheet at each epic's merge gate. Those halts are load-bearing — they caught a keypress-timestamp bug that would have made every flipper unresponsive with the whole suite green, an untested inverted-keys table, and an unmarked non-commercial licence before the file landed — so don't pre-authorize them away; the rate falls as planning artifacts stop meeting reality for the first time.
...
BMAD's optional [`bmad-loop`](https://github.com/bmad-code-org/bmad-loop) module is a Python orchestrator that also drives `bmad-build-auto`, dispatching from a `bmad-spec` folder (`SPEC.md` + `stories.yaml`). `/epic-cycle` dispatches from `epics.md` + `sprint-status.yaml` and adds the branching doctrine, architecture-decision gates, lead-side smoke, the Opus code-review gate, and per-epic worktrees. Both can be installed, but never run both over the same stories — pick one per epic. They must also never share a `deferred-work.md`: both use `### DW-n:` headings, but `bmad-loop`'s ledger records `status: open|done`, `gate:`, and `resolution:` fields while this kit's records an append-only trailer of `status= owner= by=` lines — each tool's sweep would misread the other's entries.
